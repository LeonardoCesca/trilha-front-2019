let months = [
  ...
  sentEmails = [],
  intervalEmail = window.setInterval(getNewEmail, 15000),
  intervalNewEmail = window.setInterval(getNewEmails, 60000),
  html = "";
...
const EMAIL_PREVIEW = document.getElementsByClassName("email-list");
const DIV_CONTATOS = document.getElementById("hangout-talk");

USERS.forEach(user => {
  html += `
      <div class="hangout-talk-profile" onClick='setChat(${JSON.stringify(
        user
      )})'>
        <img src=${
          user.picture
        } title="User" alt="User" class="profile-photo" />
...
DIV_CONTATOS.innerHTML = html;

$("#square").click(function() {
  if (
    $(this).attr("src") ==
    "assets/images/icons/check_box_outline_blank_black_20dp.png"
  ) {
    $(this).attr("src", "assets/images/icons/mark-check.png");
    $(".square")
      .attr("src", "assets/images/icons/mark-check.png")
      .css("opacity", "1.2");
    ...
    $(".square")
      .attr("src", "assets/images/icons/check_box_outline_blank_black_20dp.png")
      .css("opacity", "0.2");
...
$("body").click(function(event) {
  if (
    !$("label.search-email-box").is(event.target) &&
    !$("input.search-email").is(event.target)
  ) {
...
function getNewEmails() {
  ...
  EMAIL_PREVIEW[0].innerHTML = htmlEmails;
}
...
function getSentEmails() {
  ...
  EMAIL_PREVIEW[0].innerHTML = htmlSentEmails;
}
